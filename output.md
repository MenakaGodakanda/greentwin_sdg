...
══════════════════════════════════════════════
  ...
  Kuala Lumpur  (MYS)
    electrification_rate              [API, published]
    clean_cooking_access              [API, published]
    safe_water_access                 [published]
    urban_population_pct              [API]
    pm25_national_mean                [API, published]
...
── Step A: Generating full-dataset predictions (all cities)
  Feature matrix loaded: 480 rows  cities=['Bangkok', 'Ho Chi Minh City', 'Jakarta', 'Kuala Lumpur', 'Manila', 'Phnom Penh', 'Singapore', 'Yangon']
